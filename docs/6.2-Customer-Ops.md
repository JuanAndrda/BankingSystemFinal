...
**← Back to:** [Login](6.1-Login.md) | **Up to:** [Menu Overview](6-Menu-Overview.md) | **Next:** [Account Operations →](6.3-Account-Ops.md)
...
- **6.2-Customer-Ops.md** - ← You are here
...
- Establishes one-to-one (customer ↔ profile) and one-to-many (customer → accounts) relationships
...
User selects option 1 from Admin Menu → Routes to `CREATE_CUSTOMER` case → Calls `customerMgr.handleCreateCustomer()`
...
#### Step 5: Data Updates - Automatic Login Account Creation

**Location:** `CustomerManager.java:333-355`

```java
// Step 5: Auto-generate username and password (AUTOMATIC - not optional)
...
// Auto-generate username from customer name (e.g., "John Doe" -> "john_doe")
String username = authManager.generateUsername(custName);

// Auto-generate temporary password (format: "Welcomexx####")
String tempPassword = authManager.generateTemporaryPassword(username);

// Step 6: Create UserAccount object (linked to this customer)
...
public boolean registerUser(User user) {
    ...
    userRegistry.add(user);  // Add to LinkedList
    logAction(user.getUsername(), user.getUserRole(), "USER_REGISTERED", "User registered");
    return true;
}
```
...
```java
CREATE_CUSTOMER("Create Customer", 1, -1, UserRole.ADMIN, MenuCategory.CUSTOMER_OPERATIONS),
//               Display Name      ↑   ↑   Admin-only     Category
//                             Admin Cust
```
...
User Input (Admin selects option 1)
    │
    ├──→ BankingSystem.runConsoleMenu()
    │         │
    │         └──→ MenuAction.fromMenuNumber(1, ADMIN) → CREATE_CUSTOMER
    │
    ├──→ CustomerManager.handleCreateCustomer()
    │         │
    │         ├──→ Auto-generate ID: C001, C002, C003...
    │         │         │
    │         │         └──→ Iterate LinkedList<Customer> (find max ID)
    │         │
    │         ├──→ Get customer name (InputValidator)
    │         │
    ...
    │         │         │
    │         │         └──→ customers.add(customer)  [LinkedList]
    │         │
    │         ├──→ Optional: Create login account
    │         │         │
    │         │         └──→ AuthenticationManager.registerUser()
    │         │                   │
    │         │                   └──→ userRegistry.add(user)  [LinkedList]
    │         │
    │         ├──→ Optional: Create profile (promptAndCreateProfile)
    │         │
    │         ├──→ Optional: Create account (promptAndCreateAccount)
    │         │
    │         └──→ Display onboarding summary
    │
    ├──→ Log to Stack<AuditLog>
    │         │
    │         └──→ auditTrail.push(log)
    │
    └──→ Return to menu
...
✓ Customer ID auto-generated: C003

→ Customer Name: Alice Johnson
✓ Customer created: Customer{id='C003', name='Alice Johnson'}

→ Create login account for this customer? (yes/no): yes
→ Username: alice

✓ Login credentials created!
  Username:           alice
  Temporary Password: Welcomeal4582
ℹ Customer must change password on first login.

→ Create profile for Alice Johnson? (yes/no): yes
✓ Profile ID auto-generated: P003
...
→ Create an account for Alice Johnson? (yes/no): yes
→ Account Type (SAVINGS/CHECKING): SAVINGS
✓ Account number auto-generated: ACC003

→ Initial Deposit Amount: 500
✓ Account created: [ACC003] Savings Account | Owner: C003 | Balance: $500.00

════════════════════════════════════
   ONBOARDING COMPLETE
════════════════════════════════════

• Customer: Alice Johnson (C003)
• Profile: P003 ✓ CREATED
• Account: CREATED ✓
...
customers (LinkedList):
  Customer{id='C003', name='Alice Johnson',
...
✓ Customer ID auto-generated: C004

→ Customer Name: Bob Smith
✓ Customer created: Customer{id='C004', name='Bob Smith'}

→ Create login account for this customer? (yes/no): no

→ Create profile for Bob Smith? (yes/no): no

════════════════════════════════════
   ONBOARDING COMPLETE
════════════════════════════════════

• Customer: Bob Smith (C004)
• Profile: SKIPPED
• Account: NOT CREATED (Optional)
...
```java
customers (LinkedList):
  Customer{id='C004', name='Bob Smith',
...
✓ Customer ID auto-generated: C005

→ Customer Name:
✗ Invalid input: Value cannot be empty.
   Please enter a valid value.

→ Customer Name: (user types 'cancel')
...
- Demonstrates one-to-one (customer ↔ profile) and one-to-many (customer → accounts) relationships
...
User selects option 2 → Routes to `VIEW_CUSTOMER_DETAILS` case → Calls `customerMgr.handleViewCustomerDetails()`
...
public void handleViewCustomerDetails() {
    ...
    if (accounts.isEmpty()) {
        ...
        for (Account acc : accounts) {
            ...
            String info = String.format("  • %s (%s) - Balance: $%.2f",
...
User Input (Admin selects option 2)
    │
    ├──→ CustomerManager.handleViewCustomerDetails()
    │         │
    │         ├──→ InputValidator.getValidatedCustomer()
    │         │         │
    │         │         ├──→ Get Customer ID (with validation)
    │         │         │
    │         │         └──→ CustomerManager.findCustomer(id)
    │         │                   │
    │         │                   └──→ Iterate LinkedList<Customer>
    │         │                             │
    ...
    │         │         ├──→ customer.getName()
    │         │         ├──→ customer.getProfile() [One-to-One]
    │         │         │         │
    │         │         │         └──→ If not null: address, phone, email
    │         │         │
    │         │         └──→ customer.getAccounts() [One-to-Many]
    │         │                   │
    │         │                   └──→ LinkedList<Account>
    │         │                             │
    │         │                             └──→ Iterate and display all
    │         │
    │         └──→ UIFormatter.print...() [Professional box format]
    │
    └──→ Return to menu (no data changes)
```
...
╟────────────────────────────────────╢
  ASSOCIATED ACCOUNTS
╟────────────────────────────────────╢
  • ACC003 (Savings) - Balance: $500.00
  • ACC007 (Checking) - Balance: $1200.50
╚════════════════════════════════════╝
```
...
User Input (Admin selects option 3)
    │
    ├──→ CustomerManager.handleViewAllCustomers()
    │         │
    │         ├──→ Check if customers.isEmpty()
    │         │         │
    │         │         └──→ If empty: Display message and return
    │         │
    │         ├──→ Display table header
    │         │
    │         └──→ For each customer in LinkedList<Customer>:
    │                   │
    │                   ├──→ Extract: ID, Name
    │                   │
    │                   ├──→ Get account count: customer.getAccounts().size()
    │                   │
    │                   ├──→ Calculate total balance:
    │                   │         │
    │                   │         └──→ For each Account in LinkedList<Account>:
    │                   │                   │
    │                   │                   └──→ Sum acc.getBalance()
    │                   │
...
╔═══════════╤═══════════════╤══════════╤═══════════════╗
║Customer ID│ Name          │ Accounts │ Total Balance ║
╟───────────┼───────────────┼──────────┼───────────────╢
║ C001      │ John Doe      │ 1        │ $1000.00      ║
║ C002      │ Jane Smith    │ 2        │ $2500.50      ║
║ C003      │ Alice Johnson │ 2        │ $1700.50      ║
║ C004      │ Bob Smith     │ 0        │ $0.00         ║
╚═══════════╧═══════════════╧══════════╧═══════════════╝
```
...
ℹ No customers found. Use option 1 (Create Customer) to add one.
```
...
User Input (Admin selects option 4)
    │
    ├──→ CustomerManager.handleDeleteCustomer()
    │         │
    │         ├──→ Retry loop: Get & validate customer ID
    │         │         │
    │         │         └──→ findCustomer(id) → LinkedList search
    │         │
    │         ├──→ Customer found → Display details
    │         │         │
    │         │         └──→ Get account count (for warning)
    │         │
    │         ├──→ confirmActionEnhanced() with cascade warning
    │         │         │
    │         │         └──→ User confirms: YES/NO
    │         │
    │         └──→ deleteCustomer(id)
    │                   │
    │                   ├──→ Copy LinkedList<Account> (avoid ConcurrentModification)
    │                   │
    │                   ├──→ For each account:
    │                   │         │
    │                   │         └──→ accountManager.deleteAccount(accNo)
    │                   │                   │
    │                   │                   └──→ Remove from LinkedList<Account>
    │                   │
...
⚠ WARNING: This will also delete all 2 associated account(s) and transaction history.
→ Are you sure you want to delete customer C003? (yes/no): yes

⏳ Deleting customer and associated accounts...
...
→ Are you sure you want to delete customer C004? (yes/no): no
ℹ Operation cancelled.
```
