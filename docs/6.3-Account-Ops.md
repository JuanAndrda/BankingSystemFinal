...
**← Back to:** [Customer Operations](6.2-Customer-Ops.md) | **Up to:** [Menu Overview](6-Menu-Overview.md) | **Next:** [Transaction Operations →](6.4-Transaction-Ops.md)
...
- **6.3-Account-Ops.md** - ← You are here
...
| **8** | - | Delete Account | `AccountManager.handleDeleteAccount()` | Admin Only | Remove account with confirmation |
...
User selects option 5 → Routes to `CREATE_ACCOUNT` → Calls `accountMgr.handleCreateAccount()`
...
- `Account` → `Customer` (via `ownerId` and `owner` reference)
- `Customer` → `Account` (via `LinkedList<Account> accounts`)
...
User Input (Admin selects option 5)
    │
    ├──→ AccountManager.handleCreateAccount()
    │         │
    │         ├──→ Get Customer ID
    │         │         │
    │         │         └──→ CustomerManager.findCustomer(id)
    │         │                   │
    │         │                   └──→ LinkedList<Customer> search - O(n)
    │         │
    │         ├──→ Get Account Type (SAVINGS or CHECKING)
    │         │         │
    │         │         └──→ InputValidator.getValidatedAccountType()
    │         │
    │         ├──→ Auto-generate Account Number (ACC001, ACC002...)
    │         │         │
    │         │         └──→ generateNextAccountNumber()
    │         │
    │         └──→ createAccount(customerId, type, accountNo)
    │                   │
    │                   ├──→ POLYMORPHISM: Create account based on type
    │                   │         │
    │                   │         ├──→ if (SAVINGS): new SavingsAccount(...)
    │                   │         └──→ if (CHECKING): new CheckingAccount(...)
    │                   │
    │                   ├──→ accountList.add(account)  [LinkedList]
    │                   │
    │                   └──→ customer.addAccount(account)
    │                             │
    ...
    │
    └──→ Display success message with polymorphic getDetails()
```
...
→ Account type (SAVINGS or CHECKING): SAVINGS
✓ Account number auto-generated: ACC005
...
```
→ Customer ID: C004
→ Account type (SAVINGS or CHECKING): CHECKING
```

**Console Output:**
```
✓ Account number auto-generated: ACC006
...
| **Source File** | `src/com/banking/managers/AccountManager.java:484-527` |
...
#### Step 1: Menu Selection
- Admin selects #6 → Routes to `VIEW_ACCOUNT_DETAILS`
- Customer selects #1 → Routes to same `VIEW_ACCOUNT_DETAILS`
...
**Location:** `AccountManager.java:484-489`
...
**Location:** `AccountManager.java:491-527`
...
User Input (Admin #6 or Customer #1)
    │
    ├──→ AccountManager.handleViewAccountDetails()
    │         │
    │         ├──→ Get Account Number
    │         │
    │         ├──→ Find Account in LinkedList<Account>
    │         │         │
    │         │         └──→ accountMgr.findAccount(accNo)
    │         │
    │         ├──→ ACCESS CONTROL CHECK
    │         │         │
    │         │         ├──→ If ADMIN: Allow all accounts
    │         │         │
    │         │         └──→ If CUSTOMER: Check ownership
    │         │                   │
    │         │                   ├──→ account.getOwner().getCustomerId() == user.getCustomerId()?
    │         │                   ├──→ YES: Allow
    │         │                   └──→ NO: Deny (error, retry)
    │         │
    │         ├──→ Extract common data (accountNo, balance, owner)
    │         │
    │         ├──→ POLYMORPHISM: instanceof check
    │         │         │
    │         │         ├──→ if (account instanceof SavingsAccount):
    │         │         │         │
    │         │         │         └──→ Downcast, get interestRate
    │         │         │
    │         │         └──→ if (account instanceof CheckingAccount):
    │         │                   │
    │         │                   └──→ Downcast, get overdraftLimit
    │         │
    │         └──→ Display with type-specific details
    │
    └──→ Return to menu
```
...
╟───────────┼──────────┼───────────────┼───────────╢
║ ACC001    │ Savings  │ John Doe      │ $1000.00  ║
║ ACC002    │ Checking │ Jane Smith    │ $2500.50  ║
║ ACC003    │ Savings  │ Alice Johnson │ $500.00   ║
║ ACC004    │ Checking │ Bob Smith     │ $1200.00  ║
╚═══════════╧══════════╧═══════════════╧═══════════╝
```
...
| **Source File** | `src/com/banking/managers/AccountManager.java:529-583` |
| **Access Control** | ADMIN only |
| **Required Permission** | DELETE_ACCOUNT |

### 2. Purpose and Functionality

Deletes an account from the system with user confirmation and **bidirectional cleanup**.
...
- Maintains referential integrity through bidirectional cleanup
...
#### Step 4: Business Logic - User Confirmation
...
if (!confirm) {
    UIFormatter.printInfo("Account deletion cancelled.");
    return;
}
```
...
→ Are you sure you want to delete account ACC005? (y/n): n

ℹ Account deletion cancelled.
```
...
ℹ Current overdraft limit: $500.00

→ New overdraft limit: 1000

✓ Overdraft limit updated to: $1000.00
  Account: ACC006
...
ℹ Overdraft limits only apply to Checking accounts.
```
