...
**← Back to:** [Account Operations](6.3-Account-Ops.md) | **Up to:** [Menu Overview](6-Menu-Overview.md) | **Next:** [Profile & Reports →](6.5-Profile-Reports.md)
...
- **6.4-Transaction-Ops.md** - ← You are here
...
- Admin selects #10 → `DEPOSIT_MONEY`
- Customer selects #2 → `DEPOSIT_MONEY`
...
// 2. Add transaction to Stack<Transaction> in Account
account.addTransaction(tx);  // transactionHistory.push(tx)
...
2. `Stack<Transaction>` in Account - transaction pushed (LIFO)
...
public boolean deposit(String accountNo, double amount) {
    ...
    try {
        ...
        // Add to history
        account.addTransaction(tx);  // Stack.push()
...
User Input (Admin #10 or Customer #2)
    ...
    │         ├──→ Get Account Number
    │         │         │
    │         │         └──→ Find in LinkedList<Account>
    │         │
    │         ├──→ ACCESS CONTROL: canAccessAccount()?
    │         │         │
    │         │         ├──→ Admin: TRUE (all accounts)
    │         │         └──→ Customer: Check ownership
    │         │                   │
    │         │                   └──→ account.ownerId == user.customerId?
    │         │
    │         ├──→ Get Amount (validated positive, non-zero)
    │         │
    │         └──→ deposit(accountNo, amount)
    │                   │
    │                   ├──→ new Transaction(TX001, DEPOSIT, amount)
    │                   │
...
→ Account to deposit to: ACC005
✓ Account found: ACC005 (Alice Johnson)

→ Amount to deposit: 500

✓ Deposit processed: TX012
...
**Console Input/Output:**
```
→ Account to deposit to: ACC001

✗ Access denied. You can only perform transactions on your own accounts.
  Please select one of your accounts from the list above.

→ Try again with a different account? (yes/no): no
...
User Input (Admin #11 or Customer #3)
    │
    ├──→ TransactionProcessor.handleWithdraw()
    │         │
    │         ├──→ Get Account (access-controlled)
    │         │
    │         ├──→ Get Amount
    │         │
    │         └──→ withdraw(accountNo, amount)
    │                   │
    │                   ├──→ new Transaction(TX013, WITHDRAW, amount)
    │                   │
    │                   ├──→ POLYMORPHIC CALL: account.withdraw(amount)
    │                   │         │
    ...
    │                   │                   │
    │                   │                   └──→ Check: amount <= (balance + overdraft)?
    │                   │
    │                   ├──→ If successful:
    │                   │     tx.status = "COMPLETED"
    │                   │     account.addTransaction(tx)
    ...
    │                   └──→ If failed:
    │                         tx.status = "FAILED"
    │                         account.addTransaction(tx)  // Still recorded!
    │
    └──→ Log to audit trail
```
...
**Console Input/Output:**
```
→ Account to withdraw from: ACC005 (Savings)
→ Amount to withdraw: 500

✓ Withdrawal processed: TX013
...
**Console Input/Output:**
```
→ Account to withdraw from: ACC006 (Checking, $500 balance, $500 overdraft)
→ Amount to withdraw: 700

✓ Withdrawal processed: TX014

✓ Withdrawal successful!
  Amount: $700.00
  Account: ACC006
  New Balance: $-200.00  ← Negative balance (within overdraft)
```
...
**Console Input/Output:**
```
→ Account to withdraw from: ACC005 (Savings, $100 balance)
→ Amount to withdraw: 500

✗ Insufficient funds or withdrawal failed
...
→ Account to transfer FROM: ACC005
→ Destination account (to transfer TO): ACC006
→ Amount to transfer: 300

✓ Transfer processed: TX016
...
ACC006.transactionHistory contains TX016  ← Same transaction in both!
```
...
- Shows data structure conversion (LinkedList → Stack)
...
public abstract class Account {
    ...
    public void addTransaction(Transaction t) {
        this.transactionHistory.push(t);  // O(1) push - LIFO order
    }
...
**Algorithm:**
1. **Storage:** Stack<Transaction> directly stores transactions in LIFO order
2. **Add:** New transactions pushed to top of stack (most recent first)
3. **Retrieve:** Direct access to Stack - no conversion needed
...
Stack top → [TX005, TX004, TX003, TX002, TX001] ← Stack bottom
...
╟───────┼──────────┼───────────┼───────────╢
║ TX016 │ TRANSFER │ $300.00   │ COMPLETED ║  ← Most recent
║ TX013 │ WITHDRAW │ $500.00   │ COMPLETED ║
║ TX012 │ DEPOSIT  │ $500.00   │ COMPLETED ║
║ TX008 │ DEPOSIT  │ $1000.00  │ COMPLETED ║
║ TX005 │ DEPOSIT  │ $250.00   │ COMPLETED ║  ← Oldest
╚═══════╧══════════╧═══════════╧═══════════╝
```
...
```java
// Step 1: Get Stack directly from Account (no conversion needed)
Stack<Transaction> txStack = account.getTransactionHistory();
// Stack already in LIFO order: [TX016, TX013, TX012, TX008, TX005]
//                               ↑ Top (most recent)

// Step 2: Clone stack to preserve original data
@SuppressWarnings("unchecked")
Stack<Transaction> tempStack = (Stack<Transaction>) txStack.clone();

// Step 3: Pop and display from cloned stack
while (!tempStack.isEmpty()) {
    Transaction tx = tempStack.pop();  // TX016, TX013, TX012...
    print(tx);
}
```
