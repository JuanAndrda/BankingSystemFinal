...
**← Back to:** [Transaction Operations](6.4-Transaction-Ops.md) | **Next:** [Security & Session Management →](6.6-Security-Session.md)
...
- **6.5-Profile-Reports.md** - ← You are here
...
- **One-to-One Relationship:** Customer ↔ CustomerProfile bidirectional link
...
| **OOP Concept** | **One-to-One Relationship** (Customer ↔ CustomerProfile) |
...
**Location:** `BankingSystem.java:466` → `CustomerManager.java:196`
...
**Location:** `CustomerManager.java:165-194`
...
public boolean createOrUpdateProfile(String customerId, String profileId,
    ...
    try {
        ...
        // Profile constructor already set Profile → Customer (bidirectional)
...
public void setProfile(CustomerProfile profile) {
    this.profile = profile;           // Customer → Profile
    if (profile != null) {
        profile.setCustomer(this);    // Profile → Customer (bidirectional)
    }
}
...
Customer C001:
  - customerId: "C001"
  - name: "Alice Johnson"
  - profile: CustomerProfile@P001  ← ONE-TO-ONE LINK
      ├─ profileId: "P001"
      ├─ address: "123 Main St"
      ├─ phone: "5551234567"
      ├─ email: "alice@example.com"
      └─ customer: Customer@C001  ← BIDIRECTIONAL
  - accounts: []
```

#### Step 6: User Feedback (Console Output)
**Location:** `CustomerManager.java:182-188`

```
✓ Profile ID auto-generated: P001
✓ Profile created/updated for customer: Alice Johnson
Profile[P001] - Address: 123 Main St, Phone: 5551234567, Email: alice@example.com
ℹ One-to-one relationship successfully established!
```
...
**File:** `CustomerManager.java:142-163`
...
**File:** `CustomerManager.java:100-123`
...
```
┌────────────────────────────────────────────────────────────────────┐
│              CREATE/UPDATE CUSTOMER PROFILE FLOW                    │
└────────────────────────────────────────────────────────────────────┘

Admin selects option 14
    │
    ├──→ Prompt: Customer ID (C###)
    │         ├──→ Validate format (Regex)
    │         └──→ Find Customer in LinkedList<Customer>
    │
    ├──→ Check: customer.getProfile() != null?
    │         ├──→ YES: Confirm replacement
    │         └──→ NO: Continue
    │
    ├──→ Auto-generate Profile ID
    │         └──→ Scan all customers' profiles, find max ID, increment
    │
    ├──→ Prompt: Address, Phone, Email
    │         ├──→ Validate Phone (min 10 digits)
    │         └──→ Validate Email (contains @)
    │
    ├──→ Create CustomerProfile object
    │         └──→ new CustomerProfile(profileId, address, phone, email)
    │
    ├──→ Establish One-to-One Relationship
    │         ├──→ customer.setProfile(profile)      [Customer → Profile]
    │         └──→ profile.setCustomer(customer)     [Profile → Customer]
    │
...
→ Customer ID (to link profile to): C001
✓ Profile ID auto-generated: P001

→ Address: 123 Main Street, Springfield
→ Phone (min 10 digits): 5551234567
...
Customer C001:
  ...
  │     ├─ phone: "5551234567"
  │     ├─ email: "alice.johnson@example.com"
  │     └─ customer: Customer@C001  ← BIDIRECTIONAL LINK
  └─ accounts: []
```
...
```java
profile: CustomerProfile@P001
  ├─ address: "456 Oak Avenue, Shelbyville"  ← UPDATED
...
→ Customer ID (to link profile to): C999
✗ Customer not found
...
✗ Invalid email format. Email must contain @ and have text before/after.
→ Email: alice@example.com
...
public Customer getValidatedCustomerWithProfile() {
    Customer customer = this.getValidatedCustomer("✗ Customer not found.");
...
```
┌────────────────────────────────────────────────────────────────────┐
│                  UPDATE PROFILE INFORMATION FLOW                    │
└────────────────────────────────────────────────────────────────────┘
...
Admin selects option 15
    ...
    │         └─ [0] Cancel
    │
    ├──→ User selects field (1, 2, or 3)
    │
    ├──→ Prompt for new value
    │         ├──→ Validate Phone (min 10 digits) OR
    │         └──→ Validate Email (contains @)
    │
    ├──→ Update selected field
    │         └──→ profile.setAddress() / setPhone() / setEmail()
    │
    └──→ Display success message
```
...
→ Choice: 2
→ New Phone (min 10 digits): 5559876543
...
| **Source File** | `src/com/banking/managers/AccountManager.java:328-377` |
...
**Location:** `AccountManager.java:328-377`
...
```
LinkedList<Account>:
├─ SavingsAccount@ACC001  (balance: $1000.00, rate: 3%)
├─ CheckingAccount@ACC002 (balance: $500.00)   ← SKIPPED
└─ SavingsAccount@ACC003  (balance: $2000.00, rate: 3%)
```

**After:**
```
LinkedList<Account>:
├─ SavingsAccount@ACC001  (balance: $1030.00)  ← +$30 interest
├─ CheckingAccount@ACC002 (balance: $500.00)   ← UNCHANGED
└─ SavingsAccount@ACC003  (balance: $2060.00)  ← +$60 interest
```
...
╔═══════════════════════════════════════════════════════════════╗
║ Account No   Old Balance   New Balance   Interest Earned      ║
╠═══════════════════════════════════════════════════════════════╣
║ ACC001       $1000.00       $1030.00      $30.00              ║
║ ACC003       $2000.00       $2060.00      $60.00              ║
╚═══════════════════════════════════════════════════════════════╝
...
#### Step 7: Audit Logging
**Location:** `AccountManager.java:375-376`
...
```
┌────────────────────────────────────────────────────────────────────┐
│                  APPLY INTEREST (BULK OPERATION) FLOW               │
└────────────────────────────────────────────────────────────────────┘

Admin selects option 16
    │
    ├──→ Iterate LinkedList<Account>
    │         │
    │         └──→ For each Account:
    │                   │
    │                   ├──→ Runtime Type Check: acc instanceof SavingsAccount?
    │                   │         │
    │                   │         ├──→ YES: Downcast to SavingsAccount
    │                   │         │       │
    │                   │         │       ├──→ Store oldBalance
...
```
╔════════════════════════════════════╗
║ APPLY INTEREST (All Savings Accounts) ║
╚════════════════════════════════════╝
...
╠═══════════════════════════════════════════════════════════════╣
║ ACC001       $1000.00       $1030.00      $30.00              ║
║ ACC003       $5000.00       $5150.00      $150.00             ║
║ ACC005       $250.00        $257.50       $7.50               ║
╚═══════════════════════════════════════════════════════════════╝
...
**Data Changes:**
```
SavingsAccount ACC001: $1000.00 → $1030.00  (+$30.00)
CheckingAccount ACC002: $500.00 → $500.00   (SKIPPED)
SavingsAccount ACC003: $5000.00 → $5150.00  (+$150.00)
CheckingAccount ACC004: $1200.00 → $1200.00 (SKIPPED)
SavingsAccount ACC005: $250.00 → $257.50    (+$7.50)
```
...
ℹ No savings accounts found. Create accounts using option 5 and select SAVINGS type.
```
...
| **Source File** | `src/com/banking/managers/AccountManager.java:253-296` |
...
**Location:** `AccountManager.java:180-207, 253-296`
...
```
LinkedList<Account>:
  Index 0: Account[ACC001] Owner: "Alice Johnson"    Balance: $1000  ← Moved from index 1
  Index 1: Account[ACC004] Owner: "Bob Smith"        Balance: $750   ← Moved from index 2
  Index 2: Account[ACC002] Owner: "Charlie Brown"    Balance: $2000  ← Moved from index 3
  Index 3: Account[ACC003] Owner: "Zoe Williams"     Balance: $500   ← Moved from index 0
```

**Algorithm Analysis:**
- **Time Complexity:** O(n²) worst case (already sorted in reverse)
...
**File:** `AccountManager.java:180-207`
...
private void insertionSortByName(LinkedList<Account> accountList) {
    ...
    for (int i = 1; i < accountList.size(); i++) {
        ...
        while (j >= 0) {
            ...
            if (currentName.compareToIgnoreCase(compareName) < 0) {
                // Current comes before compare → shift compare to the right
                accountList.set(j + 1, compareAccount);
                j--;  // Move left to continue searching
            } else {
                // Found correct position → stop searching
                break;
            }
        }
...
Pass 1 (i=1, current="Alice"):
  Sorted: [Zoe]
  Compare: Alice < Zoe? YES → Shift Zoe right
  Result: [Alice, Zoe, Bob, Charlie]

Pass 2 (i=2, current="Bob"):
  Sorted: [Alice, Zoe]
  Compare: Bob < Zoe? YES → Shift Zoe right
  Compare: Bob < Alice? NO → Insert Bob at index 1
  Result: [Alice, Bob, Zoe, Charlie]

Pass 3 (i=3, current="Charlie"):
  Sorted: [Alice, Bob, Zoe]
  Compare: Charlie < Zoe? YES → Shift Zoe right
  Compare: Charlie < Bob? NO → Insert Charlie at index 2
  Result: [Alice, Bob, Charlie, Zoe]

Final: [Alice, Bob, Charlie, Zoe] ✓ Sorted!
```
...
```
┌────────────────────────────────────────────────────────────────────┐
│                   SORT ACCOUNTS BY NAME FLOW                        │
└────────────────────────────────────────────────────────────────────┘

Admin selects option 17
    │
    ├──→ Display BEFORE table
    │         ├─ Iterate accountList (original order)
    │         └─ Print: Account No | Type | Owner | Balance
    │
    ...
    │                   ├──→ currentName = currentAccount.getOwner().getName()
    │                   │
    │                   └──→ For j = i-1 down to 0:
    │                             │
    │                             ├──→ compareName = accountList[j].getOwner().getName()
    │                             │
    │                             ├──→ If currentName < compareName (alphabetically):
    │                             │       ├─ Shift accountList[j] to right
    │                             │       └─ Continue left (j--)
    │                             │
    │                             └──→ Else: Break (found position)
    │                             │
    │                             └──→ Insert currentAccount at position j+1
    │
    ├──→ Display AFTER table
    │         ├─ Iterate accountList (sorted order)
    │         └─ Print: Account No | Type | Owner | Balance
    │
    ├──→ Log to Audit Trail
    │         └──→ "Sorted X accounts by customer name"
    │
    └──→ Display success message
```
...
╔═══════════════════════════════════════════════════════════╗
║ Account No   Type       Owner             Balance         ║
╠═══════════════════════════════════════════════════════════╣
║ ACC003       Savings    Zoe Williams      $500.00         ║
║ ACC001       Checking   Alice Johnson     $1000.00        ║
║ ACC004       Savings    Bob Smith         $750.00         ║
║ ACC002       Checking   Charlie Brown     $2000.00        ║
╚═══════════════════════════════════════════════════════════╝
...
AFTER:  [Alice, Bob, Charlie, Zoe]  ← Alphabetical order
```
...
| **Source File** | `src/com/banking/managers/AccountManager.java:298-333` |
...
**What it does:**
- Sorts all accounts by balance (highest → lowest)
...
- Descending order (highest first: $5000 → $100)
...
**Location:** `AccountManager.java:222-246`
...
```
LinkedList<Account>:
  Index 0: Account[ACC002] Balance: $5000.00  ← Highest (moved from index 1)
  Index 1: Account[ACC003] Balance: $1000.00  ← (moved from index 2)
  Index 2: Account[ACC004] Balance: $750.00   ← (moved from index 3)
  Index 3: Account[ACC001] Balance: $250.00   ← Lowest (moved from index 0)
```
...
**File:** `AccountManager.java:298-333`
...
Admin selects option 18
    ...
    ├──→ Display BEFORE table
    │
    ├──→ Run Insertion Sort Algorithm (DESCENDING)
    │         │
    │         └──→ For i = 1 to n-1:
    │                   │
    │                   ├──→ currentBalance = accountList[i].getBalance()
    │                   │
    │                   └──→ For j = i-1 down to 0:
    │                             │
    │                             ├──→ compareBalance = accountList[j].getBalance()
    │                             │
    │                             ├──→ If currentBalance > compareBalance:
    │                             │       ├─ Shift accountList[j] to right
    │                             │       └─ Continue left (j--)
...
║ Account No   Type       Owner             Balance         ║
╠═══════════════════════════════════════════════════════════╣
║ ACC001       Savings    Alice Johnson     $250.00         ║
║ ACC002       Checking   Bob Smith         $5000.00        ║
║ ACC003       Savings    Charlie Brown     $1000.00        ║
║ ACC004       Checking   Zoe Williams      $750.00         ║
╚═══════════════════════════════════════════════════════════╝
...
AFTER:  [$5000, $1000, $750, $250]  ← Descending order
```
...
Stack<AuditLog> auditTrail:
  ┌─────────────────────────────────────────┐
  │ AuditLog[5] ← TOP (newest)              │
  │ 2025-12-04 15:30:22 | admin | DEPOSIT   │
  ├─────────────────────────────────────────┤
  │ AuditLog[4]                             │
  │ 2025-12-04 15:28:10 | alice | WITHDRAW  │
  ├─────────────────────────────────────────┤
  │ AuditLog[3]                             │
  │ 2025-12-04 15:25:00 | admin | CREATE_ACC│
  ├─────────────────────────────────────────┤
  │ AuditLog[2]                             │
  │ 2025-12-04 15:20:45 | alice | LOGIN_OK  │
  ├─────────────────────────────────────────┤
  │ AuditLog[1] ← BOTTOM (oldest)           │
  │ 2025-12-04 15:15:00 | admin | LOGIN_OK  │
  └─────────────────────────────────────────┘
```

**Display Order (LIFO - Newest First):**
```
Row 1: 2025-12-04 15:30:22 | admin | DEPOSIT       ← Newest
...
Row 5: 2025-12-04 15:15:00 | admin | LOGIN_OK      ← Oldest
```
...
**Immutability:** No setters → audit logs cannot be modified after creation (security feature).
...
```
┌────────────────────────────────────────────────────────────────────┐
│                     VIEW AUDIT TRAIL FLOW                           │
└────────────────────────────────────────────────────────────────────┘

Admin selects option 19
    │
    ├──→ Check: auditTrail.isEmpty()?
    │         ├──→ YES: Display "Audit trail is empty"
    │         └──→ NO: Continue
    │
    ├──→ Display table header
    │
    ...
    │         ├──→ Create temp stack
    │         │
    │         ├──→ Pop all logs to temp stack (reverse order)
    │         │         │
    │         │         └──→ while (!auditTrail.isEmpty()):
    │         │                   tempStack.push(auditTrail.pop())
    │         │
    │         └──→ Display and restore logs
    │                   │
    │                   └──→ while (!tempStack.isEmpty()):
    │                             │
    │                             ├──→ log = tempStack.pop()
    │                             ├──→ Display log details
    │                             └──→ auditTrail.push(log)  ← Restore
    │
...
╠═════════════════════════════════════════════════════════════════════════════════════════════════════════════╣
║ 2025-12-04 16:45:30 admin      ADMIN     DEPOSIT            Account: ACC001 Amount: $500                    ║
║ 2025-12-04 16:40:15 alice      CUSTOMER  WITHDRAW           Account: ACC002 Amount: $200                    ║
║ 2025-12-04 16:35:00 admin      ADMIN     CREATE_ACCOUNT     Account ID: ACC005 Type: SAVINGS                ║
║ 2025-12-04 16:30:22 admin      ADMIN     CREATE_CUSTOMER    Customer ID: C003 Name: Charlie Brown           ║
║ 2025-12-04 16:25:10 bob        CUSTOMER  TRANSFER           From: ACC003 To: ACC004 Amount: $100            ║
║ 2025-12-04 16:20:45 alice      CUSTOMER  LOGIN_SUCCESS      User logged in successfully                     ║
║ 2025-12-04 16:15:00 admin      ADMIN     LOGIN_SUCCESS      User logged in successfully                     ║
║ 2025-12-04 16:10:30 hacker     SYSTEM    LOGIN_FAILED       Invalid credentials for: hacker                 ║
╚═════════════════════════════════════════════════════════════════════════════════════════════════════════════╝
...
Display Order (LIFO - Newest First):
  ...
  Row 8: LOGIN_FAILED      ← BOTTOM (oldest)
```
...
ℹ Audit trail is empty. No actions have been logged yet.
```
...
- **[1-OOP-Analysis.md](1-OOP-Analysis.md)** - One-to-One relationship (Customer ↔ Profile)
