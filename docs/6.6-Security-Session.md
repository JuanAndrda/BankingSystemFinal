...
**← Back to:** [Profile Operations & Reports](6.5-Profile-Reports.md) | **↑ Up to:** [Menu Overview](6-Menu-Overview.md)
...
- **6.6-Security-Session.md** - ← You are here
...
| **Source File** | `src/com/banking/BankingSystem.java:635-692` |
...
**Location:** `BankingSystem.java:635`
...
**Location:** `BankingSystem.java:639-642`
...
**Location:** `BankingSystem.java:644-676`
...
while (true) {  // OUTER RETRY LOOP
    ...
    while (true) {
        ...
        if (newUser != null) {
            ...
            System.out.println("⚠ For security, you will be logged out. Please log in again with your new password.");
...
LinkedList<User> userRegistry:
  Index 0: Admin("admin", "admin123")              ← currentUser
...
```
LinkedList<User> userRegistry:
  Index 0: Admin("admin", "newPass456")            ← NEW OBJECT (replaced)
  Index 1: UserAccount("alice", "alice123", "C001")
  Index 2: UserAccount("bob", "bob123", "C002")

currentUser = null  ← LOGGED OUT (security)
...
**Location:** `BankingSystem.java:668-676`

```
✓ Password changed successfully!
⚠ For security, you will be logged out. Please log in again with your new password.
...
**File:** `BankingSystem.java:635-692`
...
User selects option 21 (Admin) or 6 (Customer)
    ...
    │         └──→ Validate cancellation ("back")
    │
    ├──→ Prompt: Enter new password (min 4 chars)
    │         ├──→ Validate length >= 4
    │         └──→ Validate cancellation
    │
    ├──→ Call AuthenticationManager.changePassword()
    │         │
    │         ├──→ Find User in LinkedList<User> userRegistry
    ...
    │         │
    │         ├──→ Create New User Object (IMMUTABLE PATTERN)
    │         │         ├─ instanceof Admin? → new Admin(username, newPassword)
    │         │         └─ instanceof UserAccount? → new UserAccount(username, newPassword, linkedCustomerId)
    │         │
    │         ├──→ Replace in registry
    │         │         └──→ userRegistry.set(index, newUser)
    │         │
    │         ├──→ Log to Audit Trail
...
→ Enter new password (min 4 characters): newSecurePass456
...
AFTER:
  userRegistry[0] = Admin("admin", "newSecurePass456")  ← NEW OBJECT
...
→ Enter current password: Welcomeal1234
→ Enter new password (min 4 characters): MyNewPassword

✓ Password changed successfully!
⚠ For security, you will be logged out. Please log in again with your new password.
✓ alice logged out successfully.
...
BEFORE:
  ...
  passwordChangeRequired = true  ← TEMP PASSWORD

AFTER:
  userRegistry[1] = UserAccount("alice", "MyNewPassword", "C001")  ← NEW OBJECT
  passwordChangeRequired = false  ← PERMANENT PASSWORD
```
...
→ Enter current password: wrongPass
→ Enter new password (min 4 characters): newPass123
...
→ Enter current password: admin123
→ Enter new password (min 4 characters): abc

✗ Password must be at least 4 characters. Please try again.

→ Enter new password (min 4 characters): abcd1234
...
**After Logout:**
```java
currentUser = null  ← SESSION CLEARED
```
...
User selects option 0 (Logout)
    ...
    ├──→ Check: currentUser != null?
    │         ├──→ YES: Continue
    │         └──→ NO: Do nothing (already logged out)
    │
    ...
    │
    ├──→ Log to Audit Trail
    │         └──→ auditTrail.push(new AuditLog("LOGOUT", ...))
    │
    ├──→ Clear Session
    │         └──→ currentUser = null
    │
...
if (action == MenuAction.EXIT) {
    ...
    if (bankingSystem.getCurrentUser() != null) {
        System.out.println("⚠ You are still logged in. Logging out...");
...
⚠ You are still logged in. Logging out...
✓ admin logged out successfully.
